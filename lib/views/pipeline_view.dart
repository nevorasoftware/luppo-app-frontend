...
class _PipelineViewState extends State<PipelineView> {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Metrics row / column depending on screen width
                if (isMobile)
                  Column(
                    children: [
                      StatCard(
                ...
                // If wide desktop: horizontal Kanban columns. If mobile/narrow: vertical list downwards.
                if (isDesktop)
                  SizedBox(
                    height: 580,
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) => _buildStageColumn(_stages[index]),
                    ),
                  )
                else
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: _stages.map((stage) => _buildStageVertical(stage)).toList(),
                  ),
              ],
            ),
          ),
  ...
  // Vertical layout for stages (Mobile and narrow screens)
  Widget _buildStageVertical(String stage) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Content: cards or empty message
          if (oppsInStage.isEmpty)
            const Padding(
            ...
            ListView.separated(
              ...
              itemBuilder: (context, idx) => _buildOpportunityCard(oppsInStage[idx]),
            ),
        ],
      ),
    );
  ...
  // Horizontal column layout (Wide desktop screens)
  ...
  Widget _buildOpportunityCard(OpportunityDTO opp) {
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: Colors.white,
